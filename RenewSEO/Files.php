...
class Files
{
    ...
    private static function writeFile(string $relative, string $content): void
    {
        ...
        if (is_file($path) && !self::canReplaceFile($relative, $path)) {
    ...
    private static function removeFile(string $relative): void
    {
        ...
        if (!self::isManagedFile($relative, $path)) {
            return;
        }
    ...
    private static function isManagedFile(string $relative, string $path): bool
    {
        ...
        $managed = array_fill_keys(self::trackedFiles(Settings::load(), true), true);
    ...
    private static function canReplaceFile(string $relative, string $path): bool
    {
        $managed = array_fill_keys(self::trackedFiles(Settings::load(), true), true);
        if (isset($managed[$relative])) {
            $hashes = self::trackedHashes();
